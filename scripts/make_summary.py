...
def calculate_energy(n,label,energy):
    ...
    for c in n.iterate_components(n.one_port_components|n.branch_components):
        ...
        if c.name in n.one_port_components:
            c_energies = c.pnl.p.multiply(n.snapshot_weightings.objective,axis=0).sum().multiply(c.df.sign).groupby(c.df.carrier).sum()
        else:
...
def calculate_supply_energy(n,label,supply_energy):
    ...
    for i in bus_carriers:
        ...
        for c in n.iterate_components(n.one_port_components):
            ...
            s = c.pnl.p[items].multiply(n.snapshot_weightings.objective,axis=0).sum().multiply(c.df.loc[items,'sign']).groupby(c.df.loc[items,'carrier']).sum()
            s = pd.concat([s], keys=[c.list_name])
            s = pd.concat([s], keys=[i])
        ...
        for c in n.iterate_components(n.branch_components):
            ...
            for end in [col[3:] for col in c.df.columns if col[:3] == "bus"]:
                ...
                s = (-1)*c.pnl["p"+end][items].multiply(n.snapshot_weightings.objective,axis=0).sum().groupby(c.df.loc[items,'carrier']).sum()
                s.index = s.index+end
                s = pd.concat([s], keys=[c.list_name])
                s = pd.concat([s], keys=[i])
